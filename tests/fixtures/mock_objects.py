...
class MockFactory:
    ...
    def create_mock_config(**overrides) -> Mock:
        ...
        config.get_obsidian_vault_path.return_value = Path(
            default_config["obsidian"]["vault_path"]
        )
        config.get_sync_folder_path.return_value = (
            Path(default_config["obsidian"]["vault_path"])
            / default_config["obsidian"]["sync_folder"]
        )
        config.get_templates_folder_path.return_value = (
            Path(default_config["obsidian"]["vault_path"])
            / default_config["obsidian"]["templates_folder"]
        )
        config.get_backup_folder_path.return_value = Path(
            default_config["sync"]["backup_folder"]
        )
        config.get_kindle_email.return_value = default_config["kindle"]["email"]
        config.get_approved_senders.return_value = default_config["kindle"][
            "approved_senders"
        ]
        ...
        config.get_markdown_config.return_value = default_config["processing"][
            "markdown"
        ]
    ...
    def create_mock_pdf_converter() -> Mock:
        ...
        converter._process_markdown = Mock(
            return_value="<html><body>Processed content</body></html>"
        )
        return converter
    ...
    def create_mock_pdf2image() -> Mock:
        ...
        mock_pdf2image.convert_from_path.return_value = (
            MockFactory.create_mock_pdf_images()
        )
...
class MockDataGenerator:
    ...
    def generate_markdown_content(
        title: str = "Test Document", paragraphs: int = 3
    ) -> str:
