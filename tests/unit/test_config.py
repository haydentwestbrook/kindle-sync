...
class TestConfig:
    ...
    def test_validate_missing_vault_path(self, config: Config):
        ...
        with patch.object(config, 'get_obsidian_vault_path', return_value=Path('/non/existent/path')):
    ...
    def test_validate_missing_smtp_config(self, config: Config, obsidian_vault: Path):
        ...
        with patch.object(config, 'get_obsidian_vault_path', return_value=obsidian_vault):
            with patch.object(config, 'get_smtp_config', return_value={'server': '', 'username': '', 'password': ''}):
                assert config.validate() is False
