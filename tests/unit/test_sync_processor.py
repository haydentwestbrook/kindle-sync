...
class TestSyncProcessor:
    ...
    def test_start_file_watcher_error(self, config):
        ...
        with patch.object(processor.config, "validate", return_value=True):
            with patch.object(
                processor.file_watcher, "start", side_effect=Exception("Watcher error")
            ):
                result = processor.start()
    ...
    def test_stop_with_error(self, config):
        ...
        with patch.object(
            processor.file_watcher, "stop", side_effect=Exception("Stop error")
        ):
    ...
    def test_process_markdown_file_auto_convert_disabled(
        self, config, temp_dir, sample_markdown_content
    ):
    ...
    def test_process_markdown_file_send_failed(
        self, config, temp_dir, sample_markdown_content
    ):
        ...
        with patch.object(
            processor.config, "get", side_effect=lambda key, default=None: True
        ):
    ...
    def test_process_markdown_file_auto_send_disabled(
        self, config, temp_dir, sample_markdown_content
    ):
    ...
    def test_sync_from_kindle_success(self, config, temp_dir):
        ...
        synced_files = [temp_dir / "doc1.pdf", temp_dir / "doc2.pdf"]

        # Mock Kindle sync
        with patch.object(
            processor.kindle_sync, "sync_from_kindle", return_value=synced_files
        ):
            with patch.object(processor, "_process_pdf_file") as mock_process:
    ...
    def test_sync_from_kindle_mixed_files(self, config, temp_dir):
        ...
        synced_files = [temp_dir / "doc1.pdf", temp_dir / "doc2.md"]  # Non-PDF file
    ...
    def test_cleanup_old_files_success(self, config, temp_dir):
        ...
        # Mock config methods
        with patch.object(
            processor.config, "get_sync_folder_path", return_value=sync_folder
        ):
            with patch.object(
                processor.config, "get_backup_folder_path", return_value=backup_folder
            ):
                with patch.object(
