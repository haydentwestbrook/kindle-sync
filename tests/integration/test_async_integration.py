...
class TestAsyncIntegration:
    ...
    def test_health_checker_integration(self, health_checker, mock_config, db_manager):
        ...
        with patch.object(health_checker, '_check_config_paths', return_value=("healthy", "Paths OK")), \
             patch.object(health_checker, '_check_database_connection', return_value=("healthy", "DB OK")), \
             patch.object(health_checker, '_check_email_service_config', return_value=("healthy", "Email OK")), \
             patch.object(health_checker, '_check_temp_directory_access', return_value=("healthy", "Temp OK")):
    ...
    def test_error_handling_integration(self, db_manager):
        ...
        # Verify the update
        with db_manager.get_session() as session:
            updated_file = session.query(db_manager.ProcessedFile).filter_by(id=processed_file.id).first()
    ...
    def test_metrics_collector_prometheus_format(self, metrics_collector):
        ...
        assert "kindle_sync_files_processed_total{status=\"success\",file_type=\".md\"} 2.0" in metrics_str
        assert "kindle_sync_files_processed_total{status=\"failed\",file_type=\".pdf\"} 1.0" in metrics_str
        assert "kindle_sync_pdfs_generated_total 1.0" in metrics_str
        assert "kindle_sync_errors_total{error_type=\"FileProcessingError\",severity=\"high\"} 1.0" in metrics_str
