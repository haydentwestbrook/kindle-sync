...
"""
        ...
        with patch.object(processor.markdown_to_pdf, '_generate_pdf') as mock_generate_pdf:
...
"""
    ...
    def test_concurrent_file_processing_workflow(self, config, obsidian_vault):
        ...
        for i in range(10):
            ...
            md_file.write_text(f"# Concurrent Document {i}\n\nContent for document {i}.")
        ...
        with patch.object(processor.markdown_to_pdf, '_generate_pdf') as mock_generate:
            with patch.object(processor.kindle_sync, '_send_email') as mock_send:
                with patch.object(processor.kindle_sync, 'backup_file') as mock_backup:
                    ...
                    assert processor.stats['pdfs_generated'] == 1
                    assert processor.stats['pdfs_sent_to_kindle'] == 1
                    assert processor.stats['errors'] == 0  # Error is raised, not counted
    ...
    def test_kindle_sync_workflow(self, config, obsidian_vault):
        ...
        with patch.object(processor.kindle_sync, 'get_kindle_documents') as mock_get_docs:
                ...
                with patch.object(processor.kindle_sync, 'backup_file'):
                    with patch('pdf2image.convert_from_path', return_value=[Mock()]):
                        with patch('pytesseract.image_to_string', return_value="Extracted text"):
                            ...
                            assert stats['files_processed'] == 0  # Not incremented in _process_file
        ...
        with patch.object(processor.config, 'validate', return_value=True):
            with patch.object(processor.file_watcher, 'start') as mock_start:
                with patch.object(processor.file_watcher, 'stop') as mock_stop:
                    with patch.object(processor.file_watcher, 'is_alive', return_value=True):
                        ...
                        md_file.write_text("# Lifecycle Test\n\nTesting system lifecycle.")
