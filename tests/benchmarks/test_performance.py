...
from src.security.validation import FileValidator, FileValidationRequest
...
class TestPerformanceBenchmarks:
    ...
    @pytest.mark.asyncio
    async def test_async_processor_initialization_performance(self, mock_config, benchmark):
        """Benchmark AsyncSyncProcessor initialization performance."""
        def initialize_async_processor():
            with patch('src.core.async_processor.DatabaseManager'), \
                 patch('src.core.async_processor.KindleSync'), \
...
""")
    ...
    def test_file_processing_throughput(self, temp_markdown_file, mock_config, benchmark):
        """Benchmark file processing throughput."""
        with patch('src.kindle_sync.smtplib.SMTP') as mock_smtp, \
             patch('src.pdf_converter.weasyprint.HTML') as mock_html, \
             patch('src.pdf_converter.reportlab.platypus.SimpleDocTemplate') as mock_doc:
    ...
    def test_concurrent_file_validation_performance(self, temp_markdown_file, benchmark):
