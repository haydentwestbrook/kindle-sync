...
from ..core.exceptions import SecretsError, ErrorSeverity


class SecretsManager:
    """Secure secrets management with encryption at rest."""
    
    def __init__(self, key_path: Optional[Path] = None, config: Optional[Dict[str, Any]] = None):
        """
    ...
    def _load_key(self) -> bytes:
        ...
        try:
            ...
            raise SecretsError(
                f"Failed to load encryption key: {e}",
                severity=ErrorSeverity.CRITICAL
            )
    ...
    def migrate_plaintext_secrets(self, plaintext_config: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        for config_key in secrets_to_migrate:
            try:
                ...
                if value and isinstance(value, str) and not value.startswith("encrypted:"):
                    # Encrypt the secret
                    encrypted_value = self.encrypt_secret(value)
                    self._set_nested_value(migrated_config, config_key, f"encrypted:{encrypted_value}")
