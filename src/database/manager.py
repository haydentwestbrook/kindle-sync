...
class DatabaseManager:
    ...
    def record_file_processing(
        ...
        with self.get_session() as session:
            ...
            existing = (
                session.query(ProcessedFile).filter_by(file_path=file_path).first()
            )
    ...
    def add_to_queue(
        ...
        with self.get_session() as session:
            ...
            existing = (
                session.query(ProcessingQueue).filter_by(file_path=file_path).first()
            )
    ...
    def get_next_queue_item(self) -> Optional[ProcessingQueue]:
        ...
        with self.get_session() as session:
            ...
            return (
                ...
                .filter(ProcessingQueue.scheduled_for <= now)
                .order_by(
                    ProcessingQueue.priority.desc(), ProcessingQueue.queued_at.asc()
                )
                .first()
            )
    ...
    def get_processing_statistics(self, days: int = 7) -> Dict[str, Any]:
        ...
        with self.get_session() as session:
            ...
            total_files = (
                ...
                .filter(ProcessedFile.processed_at >= start_date)
                .count()
            )
            ...
            avg_time_result = (
                session.query(text("AVG(processing_time_ms)"))
                .filter(
                    ProcessedFile.processed_at >= start_date,
    ...
    def get_database_info(self) -> Dict[str, Any]:
        ...
        with self.get_session() as session:
            ...
            if "sqlite" in self.database_url:
                size_result = (
                    session.query(text("page_count * page_size as size"))
                    .from_statement(text("PRAGMA page_count, page_size"))
                    .scalar()
                )
