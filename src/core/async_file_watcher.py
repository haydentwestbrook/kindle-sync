...
class AsyncFileHandler(FileSystemEventHandler):
    ...
    def __init__(self, processing_queue: asyncio.Queue, file_filter: Optional[Callable[[Path], bool]] = None):
        """
...
class AsyncFileWatcher:
    ...
    async def start(self, watch_path: Optional[Path] = None):
        ...
        logger.info(f"Started file watcher for {watch_path} with {self.max_workers} workers")
        ...
        while self.running:
            try:
                ...
                try:
                    ...
                    logger.info(f"{worker_name} processed {file_path}: {result.success}")
    ...
    def _file_filter(self, file_path: Path) -> bool:
        ...
        try:
            max_size = self.config.get("processing.max_file_size", 50) * 1024 * 1024  # Convert MB to bytes
    ...
    async def add_file_manually(self, file_path: Path, priority: int = 0):
        ...
        if not self._file_filter(file_path):
            logger.warning(f"File {file_path} does not pass filter, not adding to queue")
            return False
    ...
    async def process_existing_files(self, watch_path: Optional[Path] = None, max_files: int = 100):
        """
