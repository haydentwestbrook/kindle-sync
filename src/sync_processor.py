...
class SyncProcessor:
    ...
    def _process_pdf_file(self, pdf_path: Path):
        ...
        try:
            ...
            # Convert to Markdown
            markdown_path = self.pdf_to_markdown.convert_pdf_to_markdown(pdf_path)
    ...
    def cleanup_old_files(self, max_age_days: int = 30) -> int:
        ...
        try:
            ...
            if sync_folder.exists():
                cleaned_count += self.kindle_sync.cleanup_old_files(sync_folder, max_age_days)
            
            # Clean backup folder
            backup_folder = self.config.get_backup_folder_path()
            if backup_folder.exists():
                cleaned_count += self.kindle_sync.cleanup_old_files(backup_folder, max_age_days)
