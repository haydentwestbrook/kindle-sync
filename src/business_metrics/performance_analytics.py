...
class PerformanceAnalytics:
    ...
    def get_metric_summary(self, metric_name: str, hours: int = 24) -> Dict[str, Any]:
        ...
        threshold = self.thresholds.get(metric_name)
        threshold_exceeded = sum(1 for v in values if threshold and v > threshold) if threshold else 0
        ...
        for metric_name, threshold in self.thresholds.items():
            ...
            if violations:
                ...
                alerts.append({
                    ...
                    "severity": "high" if max_violation > threshold * 1.5 else "medium",
                    "first_violation": min(violations, key=lambda x: x.timestamp).timestamp.isoformat(),
                    "last_violation": max(violations, key=lambda x: x.timestamp).timestamp.isoformat()
                })
    ...
    def get_optimization_recommendations(self) -> List[Dict[str, Any]]:
        ...
        for metric_name, threshold in self.thresholds.items():
            recent_measurements = list(self.measurements[metric_name])[-100:]  # Last 100 measurements
            ...
            if average_value > threshold * 0.8:  # Approaching threshold
                recommendations.append({
                    ...
                    "threshold": threshold,
                    "recommendation": self._get_optimization_recommendation(metric_name),
            ...
            if max_value > threshold * 1.2:  # Occasional spikes
                recommendations.append({
                    ...
                    "threshold": threshold,
                    "recommendation": self._get_spike_optimization_recommendation(metric_name),
                    "priority": "medium"
                })
    ...
    def _get_spike_optimization_recommendation(self, metric_name: str) -> str:
        ...
        return recommendations.get(metric_name, "Implement monitoring and throttling for this metric")
    ...
    def cleanup_old_data(self, days: int = 30):
        ...
        for metric_name in list(self.measurements.keys()):
            ...
            while (self.measurements[metric_name] and 
                   self.measurements[metric_name][0].timestamp < cutoff_date):
