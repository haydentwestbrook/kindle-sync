...
class BusinessMetricsCollector:
    ...
    def record_file_processing(self, user_id: str, file_type: str, processing_time: float, success: bool):
        """
        ...
        self.content_metrics.average_processing_time = sum(self.processing_times) / len(self.processing_times)
        ...
        if self.metrics_collector:
            self.metrics_collector.record_counter("business_files_processed_total", 1, {
                "file_type": file_type,
                "success": str(success).lower()
            })
            self.metrics_collector.record_timing("business_file_processing_duration", processing_time * 1000, {
                "file_type": file_type
            })
    ...
    def record_email_sent(self, user_id: str, success: bool):
        ...
        # Record in system metrics if available
        if self.metrics_collector:
            self.metrics_collector.record_counter("business_emails_sent_total", 1, {
                "success": str(success).lower()
            })
    ...
    def _update_session_metrics(self, session: UserSession, duration: float):
        ...
        if session.files_processed > 0:
            ...
            self.user_engagement.files_per_user = (
                (current_avg_files * (total_sessions - 1) + session.files_processed) / total_sessions
            )
    ...
    def update_daily_metrics(self):
        ...
        daily_metric = {
            ...
            "emails_sent": sum(session.emails_sent for session in self.user_sessions.values()),
            "average_processing_time": self.content_metrics.average_processing_time,
            "success_rate": (
                self.content_metrics.successful_conversions / 
                max(1, self.content_metrics.total_files) * 100
            )
        }
    ...
    def get_business_summary(self) -> Dict[str, Any]:
        ...
        return {
            "user_engagement": {
                ...
                "average_session_duration": round(self.user_engagement.average_session_duration, 2),
            ...
            "content_metrics": {
                ...
                "success_rate": round(
                    self.content_metrics.successful_conversions / 
                    max(1, self.content_metrics.total_files) * 100, 2
                ),
                "average_processing_time": round(self.content_metrics.average_processing_time, 2)
            },
    ...
    def get_trend_data(self, days: int = 30) -> List[Dict[str, Any]]:
        ...
        return [
            metric for metric in self.daily_metrics
            if metric["date"] >= cutoff_date
        ]
...
def initialize_business_metrics(metrics_collector: Optional[MetricsCollector] = None) -> BusinessMetricsCollector:
    """
